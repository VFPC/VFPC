		...
		case 5:
		{
			...
			if (round == 5) {
				if (!restFails[0]) {
					...
					returnOut[0][8] = "Failed " + RestrictionsOutput(origin_int, pos, successes, restFails[1], restFails[2]);
					returnOut[1][8] = "Failed " + RestrictionsOutput(origin_int, pos, successes, restFails[1], restFails[2]);
				}
			}
